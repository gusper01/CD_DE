...
public class LakehouseStreamingJob {
    ...
    private static final String PRODUCER_GROUP =
            "producer.config.0";
    ...
    public static void main(String[] args) throws Exception {
         ...
         * Terraform tambiÃ©n configura checkpointing cada 60 s.
         */
        ...
        Properties producerConfig =
                applicationProperties.get(PRODUCER_GROUP);

        if (producerConfig == null) {
            throw new RuntimeException(
                    "No se encontro el grupo " + PRODUCER_GROUP
                            + ". Grupos disponibles: "
                            + applicationProperties.keySet()
            );
        }

        String processedStreamName =
                producerConfig.getProperty("stream.name");

        String processedAwsRegion =
                producerConfig.getProperty("aws.region");

        if (processedStreamName == null
                || processedStreamName.isBlank()
                || processedAwsRegion == null
                || processedAwsRegion.isBlank()) {

            throw new RuntimeException(
                    "Faltan propiedades obligatorias en "
                            + PRODUCER_GROUP
         ...
         *
         * MÃ¡s adelante agregaremos stream.arn a Terraform.
         */
        ...
        // =====================================================
        // 10. KINESIS PROCESSED SINK
        // Misma AggregatedMetric que se persiste en Iceberg.
        // =====================================================

        Properties processedSinkProperties =
                new Properties();

        processedSinkProperties.put(
                AWSConfigConstants.AWS_REGION,
                processedAwsRegion
        );

        KinesisStreamsSink<AggregatedMetric> processedKinesisSink =
                KinesisStreamsSink
                        .<AggregatedMetric>builder()

                        .setKinesisClientProperties(
                                processedSinkProperties
                        )

                        .setSerializationSchema(
                                new AggregatedMetricJsonSerializationSchema()
                        )

                        .setPartitionKeyGenerator(
                                metric -> metric.sensorId
                        )
        ...
        aggregated
                .sinkTo(processedKinesisSink)
                .name("processed-kinesis-sink");
    ...
    // =========================================================
    // =========================================================
    // KINESIS PROCESSED JSON SERIALIZER
    // =========================================================

    public static class AggregatedMetricJsonSerializationSchema
            implements SerializationSchema<AggregatedMetric> {
        ...
        public byte[] serialize(
            ...
            try {
                ...
                throw new RuntimeException(
                        "Error serializando AggregatedMetric a JSON",
                        exception
                );
            }
        }
